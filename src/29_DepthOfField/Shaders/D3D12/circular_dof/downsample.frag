...
PSOut main(VSOutput input) : SV_TARGET
{
	...
	float  cocNear = TextureCoC.Sample(samplerPoint, input.UV).r;
	...
	float cocFar00 = saturate(TextureCoC.Sample(samplerPoint, texCoord00).g * 5);
	float cocFar10 = saturate(TextureCoC.Sample(samplerPoint, texCoord10).g * 5);
	float cocFar01 = saturate(TextureCoC.Sample(samplerPoint, texCoord01).g * 5);
	float cocFar11 = saturate(TextureCoC.Sample(samplerPoint, texCoord11).g * 5);
    ...
    output.DownresCoC = float2(cocNear, cocFar00);
