...
class SinglePassBokeh: public IApp
{
	...
	void Draw()
	{
		...
		beginCmd(cmd);
		{
			...
			cmdBindPipeline(cmd, pPipelineDOF);
			{
				cmdBindDescriptorSet(cmd, gFrameIndex, pDescriptorSetsDOF[DESCRIPTOR_UPDATE_FREQ_PER_FRAME]);
				cmdDraw(cmd, 3, 0);
			}
			cmdEndGpuTimestampQuery(cmd, pGpuProfiler);
	...
	void AddShaders()
	{
		...
		shaderDesc.mStages[0] = { "dof/image.vert", NULL, 0, FSR_SrcShaders };
		shaderDesc.mStages[1] = { "dof/dof.frag", NULL, 0, FSR_SrcShaders };
		addShader(pRenderer, &shaderDesc, &pShaderDOF);
	}
	...
	void RemoveShaders()
	{
		...
		removeShader(pRenderer, pShaderDOF);
	}
	...
	void AddDescriptorSets()
	{
		...
		// DoF
		{
			...
			uint        numStaticSamplers = 2;
			{
				...
				rootDesc.ppStaticSamplerNames = pStaticSamplerNames;
				rootDesc.ppStaticSamplers = pStaticSamplers;
				addRootSignature(pRenderer, &rootDesc, &pRootSignatureDOF);
			}

			DescriptorSetDesc setDesc = { pRootSignatureDOF, DESCRIPTOR_UPDATE_FREQ_NONE, 1 };
			addDescriptorSet(pRenderer, &setDesc, &pDescriptorSetsDOF[0]);
			setDesc = { pRootSignatureDOF, DESCRIPTOR_UPDATE_FREQ_PER_FRAME, gImageCount };
			addDescriptorSet(pRenderer, &setDesc, &pDescriptorSetsDOF[1]);
		}
	}
	...
	void RemoveDescriptorSets()
	{
		...
		removeRootSignature(pRenderer, pRootSignatureDOF);
		...
		for (int i = 0; i < DESCRIPTOR_UPDATE_FREQ_COUNT; ++i)
		{
			...
			if (pDescriptorSetsDOF[i])
				removeDescriptorSet(pRenderer, pDescriptorSetsDOF[i]);
		}
	}
	...
	void AddPipelines()
	{
		...
		// DoF
		{
			...
			pipelineSettings.pRootSignature = pRootSignatureDOF;
			pipelineSettings.pShaderProgram = pShaderDOF;
			pipelineSettings.pRasterizerState = pRasterDefault;

			addPipeline(pRenderer, &desc, &pPipelineDOF);
		}
	}
	...
	void AddCmds()
	{
		...
		addCmd_n(pCmdPool, false, gImageCount, &ppCmdsDOF);
	}
	...
	void RemoveCmds()
	{
		...
		removeCmd_n(pCmdPool, gImageCount, ppCmdsDOF);
