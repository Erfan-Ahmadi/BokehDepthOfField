...
public:
	...
	void Draw()
	{
		...
		Cmd* cmd = ppCmds[gFrameIndex];
		{
			...
			cmdBeginGpuFrameProfile(cmd, pGpuProfiler);
			{
				...
				cmdBindDescriptorSet(cmd, gFrameIndex, pDescriptorSets[DESCRIPTOR_UPDATE_FREQ_PER_FRAME]);

				cmdBindPipeline(cmd, pPipelineForwardPass);
				{
					for (int i = 0; i < gSceneData.meshes.size(); ++i)
					{
						Buffer* pVertexBuffers[] = { gSceneData.meshes[i]->pPositionStream, gSceneData.meshes[i]->pNormalStream };
	...
	void PrepareDescriptorSets()
	{
		for (uint32_t i = 0; i < gImageCount; ++i)
		{
			...
			updateDescriptorSet(pRenderer, i, pDescriptorSets[DESCRIPTOR_UPDATE_FREQ_PER_FRAME], 1, params);
		}
	}
